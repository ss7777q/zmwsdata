...
import type { BeastDetailResponse, BeastLineupAnalysisResponse, BeastPlayerAnalysisResponse, PetChampionStageStatsResponse } from '../lib/api';
...
import { PetChampionStageStatsTab } from '../components/beastStats/PetChampionStageStatsTab';
...
export default function BeastStats({ stageSource, detailSource, lineupSource, playerSource, loading = false }: BeastStatsProps) {
  const location = useLocation();
  const navigate = useNavigate();
  const activeTab: BeastTab = location.pathname.endsWith('/detail')
    ? 'detail'
  ...
  const [stageData, setStageData] = useState<PetChampionStageStatsResponse | undefined>(stageSource);
  ...
  useEffect(() => {
    ...
    async function loadMissing() {
      try {
        ...
        if (activeTab === 'stages' && !stageSource) {
          tasks.push(fetchDataFile<PetChampionStageStatsResponse>('pet_champion_stage_stats').then((data) => {
  ...
  }, [activeTab, stageSource, detailSource, lineupSource, playerSource]);

  if (
    loading &&
    ((activeTab === 'stages' && !stageData) ||
      (activeTab === 'detail' && !detailData) ||
      (activeTab === 'lineup' && !lineupData) ||
      (activeTab === 'players' && (!detailData || !playerData)))
  ) {
  ...
  return (
    <div className="space-y-6 pb-16">
      <div className="flex gap-2 p-1 bg-surface border border-border rounded-xl w-max max-w-[calc(100vw-2rem)] overflow-x-auto custom-scrollbar relative z-10">
        {[
          { id: 'stages', label: '赛程阶段统计 (新)', path: '/pet_champion/stages' },
          { id: 'detail', label: '历史冠军详情', path: '/pet_champion/detail' },
          ...
          <button
            ...
            className={clsx(
              'px-5 py-2.5 rounded-lg text-sm font-bold transition-all duration-300 flex items-center gap-2 relative overflow-hidden cursor-pointer',
              activeTab === tab.id
      ...
      {activeTab === 'stages' && stageData ? <PetChampionStageStatsTab source={stageData} /> : null}
