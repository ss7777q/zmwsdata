import { FileSearch, SearchX, Tag } from 'lucide-react';
    ...
    <section className="space-y-6 animate-in fade-in duration-300">
      ...
      {results.length === 0 ? (
        <div className="card !cursor-default text-center py-14 border border-dashed border-border/80">
          <div className="mx-auto w-14 h-14 rounded-2xl bg-surface flex items-center justify-center text-textSub">
            <SearchX className="w-7 h-7" />
          </div>
          <h3 className="mt-4 text-lg font-semibold text-textMain">没有找到相关内容</h3>
          <p className="mt-2 text-textSub">可以尝试搜索时装名称、所需材料名称（如: 织虹灵线、经验）等关键词。</p>
        </div>
      ) : (
          ...
          {Object.entries(groupedResults).map(([category, catResults]) => (
            <div key={category} className="space-y-4">
              <div className="flex items-center gap-2 pb-2 border-b border-border/40">
                ...
                <span className="text-xs text-textSub bg-surface px-2 py-0.5 rounded-full">
                  {catResults.length} 项
                </span>
              </div>
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 text-xs uppercase tracking-wide text-textSub">
                          <FileSearch className="w-3.5 h-3.5" />
                          <span>{result.source}</span>
                        </div>
                        <h3 className="mt-2 text-lg font-semibold text-textMain break-words">
                          <Highlight text={result.title} query={query} />
                        </h3>
                        {result.subtitle && (
                          <p className="mt-1 text-sm text-textSub/80 font-medium">
                            <Highlight text={result.subtitle} query={query} />
                          </p>
                        )}
                        {/* 实体核心数据展示 */}
                        {result.details && result.details.length > 0 && (
                          <div className="mt-4 flex flex-col gap-2 bg-surface/30 px-4 py-3 rounded-xl border border-border/40">
                            {result.details.map((detail, dIdx) => (
                              <div key={`${result.id}-d${dIdx}`} className="flex gap-4 text-sm items-start py-1 border-b border-border/20 last:border-0 last:pb-1">
                    ...
                    <div className="mt-4 space-y-2">
                      {result.matches.map((match, idx) => (
                        <div key={`${result.id}-m${idx}`} className="rounded-xl border border-border/40 bg-surface/40 px-3 py-2 flex gap-3 text-sm">
