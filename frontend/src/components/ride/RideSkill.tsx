                            ...
                            <table className="w-full text-left text-sm whitespace-nowrap">
                                <thead className="bg-surface sticky top-0 z-10 shadow-sm">
                                    <tr className="border-b border-border text-textSub text-xs uppercase tracking-wider">
                                        ...
                                        <th className="px-4 py-3 font-medium">角色达标要求</th>
                                        <th className="px-4 py-3 font-medium">{group.name}单次消耗</th>
                                        <th className="px-4 py-3 font-medium text-cta">{group.name}到本级总计</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-border/30 bg-background/30">
                                    {group.levels && (() => {
                                        ...
                                        return group.levels.map((lvl: any, idx: number) => {
                                            cumulativeCount += Number(lvl.count || 0);

                                            return (
                                                <tr key={idx} className="hover:bg-white/5 transition-colors group/row">
                                                    <td className="px-4 py-2.5 font-mono text-textMain font-medium">{lvl.level} 级</td>
