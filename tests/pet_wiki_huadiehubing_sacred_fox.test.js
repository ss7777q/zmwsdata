...
assert.strictEqual(summonArmorBuff?.qualified, null, '冰狐战士霸体应绑定在寒冰盾上，而不是技能限定 Buff');
assert.ok(mainIndex.includes('this.attachBuffs[o].destroy()'), '护盾销毁时应一并销毁附加霸体');
...
assert.ok(mechanic(summon, '属性继承').includes('技能等级的5倍'));
assert.ok(mechanic(summon, '属性继承').includes('最终整体向上取整'));
assert.ok(mechanic(summon, '属性继承').includes('对应属性的33.33%'));
assert.ok(mechanic(summon, '属性继承').includes('生命、回血取基准的66.67%'));
assert.ok(mechanic(summon, '属性继承').includes('表中固定值已换算并四舍五入保留整数'));
assert.ok(mechanic(summon, '属性继承').includes('实际结算仍使用未取整值'));
  assert.ok(mechanic(summon, '属性继承').includes('速度不继承本体'));
  assert.ok(mechanic(summon, '属性继承').includes('固定值264'));
  assert.ok(mechanic(summon, '属性继承').includes('防御、命中、闪避、暴击、韧性、幸运、守护'));
  assert.ok(mechanic(summon, '属性继承').includes('约100%'));
  assert.ok(mechanic(summon, '属性继承').includes('hpType=7'));
assert.ok(mechanic(summon, '寒冰盾与霸体').includes('护盾存在期间冰狐战士处于霸体'));
assert.ok(mechanic(summon, '寒冰盾与霸体').includes('护盾消失时霸体一并移除'));
...
assert.ok(fixedEffect(summon, '冰狐战士寒冰盾').includes('护盾存在期间附加霸体'));
assert.ok(summon.header.note.includes('护盾存在期间附加霸体'));
assert.strictEqual(summon.levels[0].metrics.find((entry) => entry.key === 'summonHp')?.label, '召唤生命固定值');
assert.strictEqual(summon.levels[0].metrics.find((entry) => entry.key === 'summonAtk')?.label, '召唤攻击固定值');
assert.strictEqual(summon.levels[0].metrics.find((entry) => entry.key === 'summonHealHp')?.label, '召唤回血固定值');
assert.strictEqual(summon.levels[0].metrics.find((entry) => entry.key === 'summonBreak')?.label, '召唤穿透固定值');
assert.strictEqual(summon.levels[0].metrics.find((entry) => entry.key === 'summonProtect')?.label, '召唤减伤固定值');
assert.strictEqual(metricDisplay(summon, 1, 'summonHp'), '353');
assert.strictEqual(metricDisplay(summon, 1, 'summonAtk'), '53');
assert.strictEqual(metricDisplay(summon, 1, 'summonHealHp'), '19');
assert.strictEqual(metricDisplay(summon, 1, 'summonBreak'), '0');
assert.strictEqual(metricDisplay(summon, 1, 'summonProtect'), '0');
assert.strictEqual(metricDisplay(summon, 47, 'summonHp'), '399754');
assert.strictEqual(metricDisplay(summon, 47, 'summonHealHp'), '19057');
assert.strictEqual(metricDisplay(summon, 60, 'summonHp'), '840300');
assert.strictEqual(metricDisplay(summon, 60, 'summonAtk'), '126039');
assert.strictEqual(metricDisplay(summon, 60, 'summonHealHp'), '39766');
assert.strictEqual(metricDisplay(summon, 60, 'summonBreak'), '10949');
assert.strictEqual(metricDisplay(summon, 60, 'summonProtect'), '10949');
