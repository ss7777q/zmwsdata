 ...
 *
 * 白虎四技能「横扫千军」是三阶段技能链：20410020401 -> 20410020402
 * -> 20410020403。前两段是位移/衔接，真实伤害在第三阶段龙卷子弹，
 * 展示按攻略口径 0.388 × 17 计算。
 */
...
const PET_SKILL = {
  ...
  SKILL_3: 20410020301,
  SKILL_4: 20410020401,
  SKILL_4_MOVE: 20410020402,
  SKILL_4_HIT: 20410020403,
  SP: 20410020501,
  PASSIVE: 20410020601,
};
...
const PET_BUFF = {
  HUNT_MARK: 194000201,
  CRIT_UP: 9001701,
  TIGER_SUPER_ARMOR: 147001001,
  BLEED: 1022501,
  SERIOUS_WOUND: 148001301,
  SP_CONTROL_IMMUNE: 146000401,
  PASSIVE_CRIT: 9001801,
  PASSIVE_DODGE: 8001401,
};
...
const SPECIAL_CONCRETE_SKILLS = new Map([
  [PET_SKILL.SKILL_4, [PET_SKILL.SKILL_4, PET_SKILL.SKILL_4_MOVE, PET_SKILL.SKILL_4_HIT]],
]);
...
const GUIDE_DAMAGE_PATCHES = new Map([
  [PET_SKILL.SKILL_4, {
    ...
    hits: 17,
    kind: "guideMultiHit",
    source: "guide:shenshouWiki:横扫千军按0.388*atk十七连击展示",
  }],
]);
...
const GUIDE_RELEASE_FRAMES = new Map([
  [PET_SKILL.ATTACK, guideRelease(1.667, "guide:shenshouWiki:白虎普攻释放1.667s")],
  [PET_SKILL.SKILL_1, guideRelease(1.2, "guide:shenshouWiki:猛虎追猎释放用时1.2s")],
  [PET_SKILL.SKILL_2, guideRelease(1.5, "guide:shenshouWiki:虎啸山林释放用时1.5s")],
  [PET_SKILL.SKILL_3, guideRelease(2.233, "guide:shenshouWiki:噬血虎爪释放用时2.233s")],
  [PET_SKILL.SKILL_4, guideRelease(2.3, "guide:shenshouWiki:横扫千军三阶段合计释放2.3s")],
  [PET_SKILL.SP, guideRelease(2.9, "guide:shenshouWiki:弑天虎爪释放用时2.9s")],
]);

const GUIDE_CD_SECONDS = new Map([
  [PET_SKILL.SKILL_1, 10],
  [PET_SKILL.SKILL_2, 20],
  [PET_SKILL.SKILL_3, 20],
  [PET_SKILL.SKILL_4, 39],
  [PET_SKILL.SP, 20],
]);
...
function mergeBuffForDisplay(displaySkillId, baseBuffId, rawBuff, engineBuff, ctx, warnings) {
  ...
  const { merged, warnings: w } = ov.mergeBuff(engineBuff, override, rawBuff, `[baihu buff ${baseBuffId} ${engineBuff.name}] `);
  warnings.push(...w);
  return merged;
}
...
function addGrowthBuffRef(growthBuffRefs, seenBuffs, displaySkillId, baseBuffId, bindSource, ctx, warnings) {
  ...
  growthBuffRefs.push({
    ...
    override,
    label: `[baihu buff ${baseBuffId} ${rawBuff1.name || ""}] `,
  });
...
function collectBaihuPassive(displaySkillId, fixedBuffs, ctx, warnings) {
  const be = ctx.beskillById.get(PET_BESKILL.WIND_GUARD);
  if (!be) warnings.push({ code: eng.WARN.MISSING_BESKILL, detail: `beskill ${PET_BESKILL.WIND_GUARD} 缺失` });
  pushPassiveBeskill(fixedBuffs, PET_BESKILL.WIND_GUARD, be?.name || "御风守护", be?.text || "提升自身暴击和闪避");

  for (const buffId of [PET_BUFF.PASSIVE_CRIT, PET_BUFF.PASSIVE_DODGE]) {
...
function collectGuideEffects(displaySkillId, pet, ctx, warnings) {
  ...
  if (displaySkillId === PET_SKILL.SKILL_1) {
    const be = ctx.beskillById.get(PET_BESKILL.HUNT_MARK);
    if (!be) warnings.push({ code: eng.WARN.MISSING_BESKILL, detail: `beskill ${PET_BESKILL.HUNT_MARK} 缺失` });
    pushPassiveBeskill(out, PET_BESKILL.HUNT_MARK, be?.name || "白虎印记", "命中后对血量最低目标附加猎食印记");
    pushFixedBuff(out, displaySkillId, PET_BUFF.HUNT_MARK, ctx.buffById.get(PET_BUFF.HUNT_MARK), "guideEffect", ctx, warnings, null);
  }

  if (displaySkillId === PET_SKILL.SKILL_2) {
    pushFixedBuff(out, displaySkillId, PET_BUFF.TIGER_SUPER_ARMOR, ctx.buffById.get(PET_BUFF.TIGER_SUPER_ARMOR), "guideEffect", ctx, warnings, null);
  }

  if (displaySkillId === PET_SKILL.SKILL_3) {
    pushFixedBuff(out, displaySkillId, PET_BUFF.SERIOUS_WOUND, ctx.buffById.get(PET_BUFF.SERIOUS_WOUND), "guideEffect", ctx, warnings, null);
  }
...
function collectBuffs(displaySkillId, concreteIds, pet, slotKind, ctx, warnings) {
  ...
  if (slotKind === "passive" && displaySkillId === PET_SKILL.PASSIVE) collectBaihuPassive(displaySkillId, fixedBuffs, ctx, warnings);
...
function extract() {
  ...
  const payload = {
    petGroup: {
      ...
      petIds: PET_IDS,
      note: "宠物技能 Wiki 使用满级作为卡片表头参考值，逐级成长见下方成长数值表；横扫千军按三阶段链路取第三阶段17段实际伤害，前两段位移衔接不计入展示伤害。",
    },
    variants,
  };

  u.saveOutput("pet_wiki_baihu", payload, {
    system: "pet_wiki",
    ...
    note: "白虎战神宠物技能 Wiki，包括普攻、主动技能、无双、被动、猎食印记、流血重伤、暴击强化和横扫千军三阶段伤害。",
  });
