 ...
 *
 * 玄武四技能「水湮八荒」的真实攻略口径是 1.06 × 5。配置动作里存在
 * 2448 子弹 maxHit=99，不能直接作为展示段数使用。
 */
...
const PET_BUFF = {
  FREEZE: 35001201,
  TENACITY_UP: 10001101,
  SHIELD_GUARD: 180000501,
  WATER_SNAKE_GUARD: 173000001,
  SLOW: 4030001,
  ATK_DOWN_ZERO: 5014001,
  GIANT: 31001701,
  SUPER_ARMOR: 147000101,
  CONTROL_IMMUNE: 146000501,
  ATK_UP: 5002901,
  DEF_UP: 6003401,
  PASSIVE_TENACITY: 10001201,
  PASSIVE_DEF: 6003501,
};
...
const GUIDE_DAMAGE_PATCHES = new Map([
  [PET_SKILL.SKILL_4, {
    ...
    hits: 5,
    kind: "guideMultiHit",
    source: "guide:shenshouWiki:水湮八荒按1.06*atk五连击展示",
  }],
]);
...
const GUIDE_RELEASE_FRAMES = new Map([
  [PET_SKILL.ATTACK, guideRelease(2.7, "guide:shenshouWiki:玄武普攻释放2.7s")],
  [PET_SKILL.SKILL_1, guideRelease(2.5, "guide:shenshouWiki:玄盾冲击释放用时2.5s")],
  [PET_SKILL.SKILL_2, guideRelease(1.233, "guide:shenshouWiki:御水神盾释放用时1.233s")],
  [PET_SKILL.SKILL_3, guideRelease(1.3, "entityCtg.time.skill3:水蛇护体动作39帧；攻略说明登场后自动释放")],
  [PET_SKILL.SKILL_4, guideRelease(1.8, "guide:shenshouWiki:水湮八荒释放用时1.8s")],
  [PET_SKILL.SP, guideRelease(1.367, "guide:shenshouWiki:神威天降释放用时1.367s")],
]);

const GUIDE_CD_SECONDS = new Map([
  [PET_SKILL.SKILL_1, 21],
  [PET_SKILL.SKILL_2, 25],
  [PET_SKILL.SKILL_3, 60],
  [PET_SKILL.SKILL_4, 21],
  [PET_SKILL.SP, 30],
]);
...
function mergeBuffForDisplay(displaySkillId, baseBuffId, rawBuff, engineBuff, ctx, warnings) {
  ...
  const { merged, warnings: w } = ov.mergeBuff(engineBuff, override, rawBuff, `[xuanwu buff ${baseBuffId} ${engineBuff.name}] `);
  warnings.push(...w);
  return merged;
}
...
function addGrowthBuffRef(growthBuffRefs, seenBuffs, displaySkillId, baseBuffId, bindSource, ctx, warnings) {
  ...
  growthBuffRefs.push({
    ...
    override,
    label: `[xuanwu buff ${baseBuffId} ${rawBuff1.name || ""}] `,
  });
...
function collectXuanwuPassive(displaySkillId, fixedBuffs, ctx, warnings) {
  const be = ctx.beskillById.get(PET_BESKILL.WATER_GUARD);
  if (!be) warnings.push({ code: eng.WARN.MISSING_BESKILL, detail: `beskill ${PET_BESKILL.WATER_GUARD} 缺失` });
  pushPassiveBeskill(fixedBuffs, PET_BESKILL.WATER_GUARD, be?.name || "御水守护", be?.text || "提升自身韧性和防御");

  for (const buffId of [PET_BUFF.PASSIVE_TENACITY, PET_BUFF.PASSIVE_DEF]) {
...
function collectGuideEffects(displaySkillId, pet, ctx, warnings) {
  ...
  if (displaySkillId === PET_SKILL.SKILL_2) {
    pushFixedBuff(out, displaySkillId, PET_BUFF.SHIELD_GUARD, ctx.buffById.get(PET_BUFF.SHIELD_GUARD), "guideEffect", ctx, warnings, null);
  }
  ...
  if (displaySkillId === PET_SKILL.SKILL_3) {
    ...
    const summonIds = asArray(summon?.mIds).filter(Boolean);
    out.push({
      ...
      value: null,
      displayText: summonIds.length
        ? "召唤水蛇；攻略说明登场1s后自动释放，不占用玄武自身行动时间"
        : "召唤水蛇；攻略说明登场1s后自动释放，不占用玄武自身行动时间",
    });
  }

  if (displaySkillId === PET_SKILL.SP) {
    for (const buffId of [PET_BUFF.SUPER_ARMOR, PET_BUFF.CONTROL_IMMUNE]) {
...
function collectBuffs(displaySkillId, concreteIds, pet, slotKind, ctx, warnings) {
  ...
  if (slotKind === "passive" && displaySkillId === PET_SKILL.PASSIVE) collectXuanwuPassive(displaySkillId, fixedBuffs, ctx, warnings);
...
function extract() {
  ...
  const payload = {
    petGroup: {
      ...
      petIds: PET_IDS,
      note: "宠物技能 Wiki 使用满级作为卡片表头参考值，逐级成长见下方成长数值表；水湮八荒按攻略与新版机制修正为5段，不按表现弹幕的99段上限展示。",
    },
    variants,
  };

  u.saveOutput("pet_wiki_xuanwu", payload, {
    system: "pet_wiki",
    ...
    note: "玄武大帝宠物技能 Wiki，包括普攻、主动技能、无双、被动、水蛇召唤、成长 buff 和水湮八荒新版5段伤害。",
  });
