 ...
 *
 * 原理:
 *   - 输出技能的攻略口径修正比 = (总固伤 / X) / CD
 *   - 因此常规技能满足 totalVal / cd ≈ X
 *   - 对同一技能等级的 totalVal / cd 聚类,最大主簇即该等级的 X
 *
 * 本模块只消费已经完成伤害段合并的 pet_wiki_*.json,不使用攻略里的倍率文本。
 * 缺 CD 或缺固伤的技能不进入样本;样本不足时只写 warning,不构造基准值。
 */
const fs = require("fs");
...
function collectSamples(warnings) {
  const samples = [];
  const files = listPetWikiFiles();

  if (!files.length) {
    pushWarning(warnings, "NO_PET_WIKI_OUTPUT", "output 目录没有 pet_wiki_*.json,无法反推宠物技能基准值");
    return samples;
  }
    ...
    for (const variant of variants) {
      ...
      for (const slot of variant.slots || []) {
        ...
        for (const levelRow of base.levels || []) {
          ...
          if (typeof totalVal !== "number" || !Number.isFinite(totalVal) || totalVal <= 0) {
            continue;
          }
...
function summarizeSkills(samples, baselines) {
  ...
  for (const sample of samples) {
    ...
    const key = `${sample.file}|${sample.petId}|${sample.slot}|${sample.skillId}`;
    if (!grouped.has(key)) {
      grouped.set(key, {
        file: sample.file,
        petId: sample.petId,
  ...
  return [...grouped.values()].map((skill) => {
    ...
    Number(a.petId || 0) - Number(b.petId || 0) ||
...
function extract() {
  ...
  u.saveOutput(OUTPUT_NAME, {
    ...
    method: "按技能等级聚类 totalVal / cd,最大主簇作为该等级 X;再计算 totalVal / X 和 (totalVal / X) / cd",
