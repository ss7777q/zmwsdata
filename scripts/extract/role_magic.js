...
const SOUL_GUIDES = {
  1: { text: '器魂被动为常驻效果：携带法宝出战时以自身不可卸属性（如称号、法宝、基础属性等）为基数百分比提升自身回血属性，装备、丹气、羽毛等系统提供的属性不计入。', kind: 'attribute', label: '回血属性提升', get: b => pct(b.attribute[0]) },
  2: { text: '器魂被动为常驻效果：提升宠物捕捉成功率。', kind: 'plain', label: '捕捉成功率提升', get: b => pct(b.attribute) },
  3: { text: '器魂被动为常驻效果：携带法宝出战时以自身不可卸属性（如称号、法宝、基础属性等）为基数百分比提升自身回魔属性，装备、丹气、羽毛等系统提供的属性不计入。', kind: 'attribute', label: '回魔属性提升', get: b => pct(b.attribute[0]) },
  4: { text: '器魂被动为常驻效果：携带法宝出战时以自身不可卸属性（如称号、法宝、基础属性等）为基数百分比提升自身闪避属性，装备、丹气、羽毛等系统提供的属性不计入。', kind: 'attribute', label: '闪避属性提升', get: b => pct(b.attribute[0]) },
  ...
  9: { text: '器魂被动为常驻效果：携带法宝出战时以自身不可卸属性（如称号、法宝、基础属性等）为基数百分比提升自身防御属性，装备、丹气、羽毛等系统提供的属性不计入。', kind: 'attribute', label: '防御属性提升', get: b => pct(b.attribute[0]) },
  10: { text: '器魂被动为常驻效果：提升自己场上宠物的攻击属性（以宠物基础攻击与宠物装备攻击为百分比提升基数，不包含局内暴击、最终增伤等后续伤害环节的直接放大）。', kind: 'petAttack' },
